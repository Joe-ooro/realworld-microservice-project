...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit ooro/reddit:latest "
                       sh "docker push ooro/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image ooro/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.148.145.252 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.14.13.84:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#2025-microservice-devsecops-cicd-alerts', //update and provide your channel name
