...
pipeline{
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.220.0.99 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.222.37.231:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#friday-devsecops-cicd-alerts', //update and provide your channel name
