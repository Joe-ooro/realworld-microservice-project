...
pipeline{
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.222.211.65 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.14.3.194:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#jo123-devsecops-cicd-alerts', //update and provide your channel name
